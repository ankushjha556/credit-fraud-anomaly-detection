...
def load_all_models():
    ...
    ae_model = FraudAutoencoderV2(29,
                   int(cfg["ae_params"]["bottleneck"]),
...
def predict_transaction(features: np.ndarray):
    """features: array of shape (1, 29) — V1..V28 + Amount"""
    feat = features.copy()
...
with st.sidebar:
    ...
    <div style="padding:0 0 1.5rem;border-bottom:1px solid rgba(255,255,255,0.05);margin-bottom:1.5rem;">
        <div style="display:flex;align-items:center;gap:12px;">
            ...
            <div>
                <div style="font-size:1rem;font-weight:800;color:#f1f5f9;letter-spacing:-0.02em;">FraudShield AI</div>
                <div style="font-size:0.62rem;color:#475569;font-family:'JetBrains Mono',monospace;letter-spacing:0.06em;">ENSEMBLE DETECTOR · v1.0</div>
            </div>
        </div>
    </div>
    """ + "".join(f"""
    ...
    <div style="background:rgba(239,68,68,0.05);border:1px solid rgba(239,68,68,0.1);
        ...
        — no fraud labels used. Fraud = high reconstruction error + short isolation path.
        V17 shows <span style="color:#ef4444;font-weight:600;">435× higher</span>
...
<div style="height:1px;background:linear-gradient(90deg,transparent,rgba(239,68,68,0.2),rgba(245,158,11,0.2),transparent);margin:0 0 2rem;"></div>
""", unsafe_allow_html=True)

# ── Demo Mode Toggle ───────────────────────────────────────────
mode = st.radio("", ["📋 Manual Input", "📁 Upload CSV"],
...
if mode == "📋 Manual Input":
    ...
    <div style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.07);
        ...
        <div style="font-size:0.62rem;font-weight:600;color:#64748b;text-transform:uppercase;
            ...
            Transaction Features (V1–V28 are PCA-transformed)
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    preset = st.selectbox("Load a preset transaction:",
        ["Custom", "Typical Normal ($22)", "High Normal ($500)",
         "Suspicious Pattern", "Known Fraud Pattern"])

    presets = {
        "Typical Normal ($22)":
            [-1.36,-0.07,2.54,1.38,-0.34,0.46,0.24,0.10,0.36,0.09,
             -0.55,-0.62,-0.99,-0.31,1.47,-0.47,0.21,0.16,0.16,-0.29,
             -0.07,-0.33,-0.96,-0.19,0.31,0.02,0.27,-0.11, 22.0],
        "High Normal ($500)":
            [1.19,0.27,-0.28,-0.11,-0.73,-0.45,-0.16,0.08,0.04,-1.54,
              0.72,-0.45,-0.35,-0.51,-0.07,-0.45,-0.44,-0.22,-0.02, 0.06,
             -0.01,-0.25,-0.19,0.05,-0.06,0.03,-0.01, 0.01,500.0],
        "Suspicious Pattern":
            [-2.31,1.95,-1.61,3.99,-0.52,1.80,-0.37,-0.25,-1.42,0.80,
             -0.29,-1.58,-0.89,-2.18,0.47,-0.68,-0.02,-1.08,-0.44,0.73,
              0.57,-0.10,-0.19,-1.18,0.65,-0.22, 0.08,-0.07, 149.62],
        "Known Fraud Pattern":
            [-3.04,-3.16,1.09,2.29,4.74,3.68,-0.49,-0.22,-4.78,-2.63,
             -4.54, 1.18,-3.43,-1.34,-0.24,-4.39,-0.26,-0.31,-0.51, 0.41,
              0.03, 0.16,-0.14,-0.07,-0.20,-0.17,-0.35,-0.04,  1.0],
    }
    ...
    for row_start in range(0, 28, cols_per_row):
        row_feats = feature_names[row_start:row_start+cols_per_row]
        row_defs  = default_vals[row_start:row_start+cols_per_row]
    ...
    if st.button("🔍 Analyze Transaction", use_container_width=True):
        with st.spinner("Running ensemble inference..."):
            if_score, ae_score = predict_transaction(features)

            # Normalize using rough training ranges
        ...
        <div style="text-align:center;padding:2rem;
                    background:{v_bg};border:2px solid {v_bdr};
                    border-radius:18px;margin:1.5rem 0;">
            <div style="font-size:2.2rem;font-weight:800;color:{sc};
        ...
        """ + "".join(f"""
            <div style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.07);
                ...
                <div style="position:absolute;top:0;left:0;right:0;height:2px;background:{bc};
                            border-radius:14px 14px 0 0;"></div>
                <div style="font-size:1.2rem;margin-bottom:0.4rem;">{ic}</div>
            ...
            ("🌲","Isolation Forest Score",f"{if_norm:.4f}","#f59e0b",min(if_norm*100,100)),
            ("🧠","Autoencoder AE Score",  f"{ae_norm:.4f}","#7c3aed",min(ae_norm*100,100)),
            ("⚡","Ensemble Risk Score",   f"{risk_pct}%",  sc,        risk_pct),
...
# ── CSV Upload Mode ────────────────────────────────────────────
    ...
    <div style="font-size:0.75rem;color:#475569;font-family:'JetBrains Mono',monospace;
                margin-bottom:1rem;">
        CSV must have columns: V1, V2, ..., V28, Amount
    </div>
    """, unsafe_allow_html=True)

    uploaded = st.file_uploader("Upload transaction CSV",
                                 type=["csv"],
    ...
    if uploaded:
        ...
        if not all(c in df.columns for c in req_cols):
            st.error("❌ Missing required columns. Need V1-V28 + Amount")
        else:
            ...
            with st.spinner(f"Analyzing {len(df)} transactions..."):
                ...
                ens = cfg["w_if"]*norm(if_scores) + cfg["w_ae"]*norm(ae_scores)
                preds = (ens >= cfg["threshold"]).astype(int)
                df["Risk Score"] = np.round(ens, 4)
                df["Verdict"]    = ["🚨 FRAUD" if p else "✅ Normal" for p in preds]
            ...
            display_cols = ["V1","V2","V3","Amount","Risk Score","Verdict"]
            st.dataframe(
            ...
            st.download_button("⬇️ Download Results CSV",
                                csv_out, "fraud_results.csv",
                                "text/csv", use_container_width=True)

# ── Footer ─────────────────────────────────────────────────────
st.markdown("""
<div style="height:1px;background:linear-gradient(90deg,transparent,rgba(239,68,68,0.15),rgba(245,158,11,0.15),transparent);margin:3rem 0 1.5rem;"></div>
